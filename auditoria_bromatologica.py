...
SECTIONS = [
    ("Instalaciones y Estructura Edilicia", "🏗️", [
...
def generate_pdf():
    ...
    pdf.set_xy(14, 7);  pdf.set_font("Helvetica","",7);  pdf.set_text_color(167,243,197)
    pdf.cell(182, 5, "SEGURIDAD ALIMENTARIA - BUENAS PRACTICAS DE MANUFACTURA", align="C")
    pdf.set_xy(14, 14); pdf.set_font("Helvetica","B",14); pdf.set_text_color(255,255,255)
    pdf.cell(182, 8, "INFORME DE VISITA BROMATOLOGICA", align="C")
    pdf.set_xy(14, 24); pdf.set_font("Helvetica","I",8);  pdf.set_text_color(203,213,225)
    pdf.cell(182, 5, "Lic. C. Anabel Marin - Food Quality & Safety Consulting", align="C")
    pdf.set_xy(14, 32); pdf.set_font("Helvetica","",7);   pdf.set_text_color(167,243,197)
    ...
    with st.expander(f"{icon}  **{sec_name}**  —  {sec_ans}/{len(items)}{sec_pct}", expanded=False):
        for idx, item in enumerate(items):
            ...
            if cur in ("Necesita Mejora", "No Cumple"):
                st.markdown("**📝 Observacion / Accion correctiva:**")
                ...
                st.markdown("**📷 Agregar fotos (se incluyen en Anexo del PDF):**")
                ...
                if uploaded:
                    ...
                    st.info(f"📎 Referencia en PDF: F-{sec_idx3+1}.{idx+1}")
            st.markdown("---")
        st.write("")
...
with col_pdf:
    if st.button("📄 Generar PDF para el Responsable del Local", use_container_width=True, type="primary"):
        ...
        fname = f"Visita_{(st.session_state.establishment or 'local').replace(' ','_')}_{fmt_date(st.session_state.audit_date).replace('/','_')}.pdf"
